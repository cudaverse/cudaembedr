.embedding_test_sce <- function(include_record = TRUE) {
  ...
  if (include_record) {
    ...
    metadata$cudacellr <- list(
      ...
      compute_stages = list(
        normalization = cudatensr::cuda_stage(
        ...
        pca_decomposition = cudatensr::cuda_stage(
...
test_that("cudacell_sce results flow directly into diffusion maps", {
  skip_if_not_installed("SingleCellExperiment")
  skip_if_not_installed("S4Vectors")
  skip_if_not_installed("cudacellr")
  skip_if(
    utils::packageVersion("cudacellr") < "0.3.0",
    "cudacellr 0.3.0 or newer is required"
  )
  counts <- outer(
    seq_len(6),
    seq_len(12),
    function(feature, cell) {
      (feature * 7L + cell * 3L + feature * cell) %% 9L
    }
  )
  ...
  processed <- cudacellr::cudacell_sce(
    sce,
    n_hvg = 5,
    n_components = 3,
    k = 3,
  ...
  expect_identical(rownames(fit$coordinates), colnames(processed))
  expect_identical(fit$parameters$reduced_dim, "CUDACELL_PCA")
  expect_identical(fit$source_class, "SingleCellExperiment")
